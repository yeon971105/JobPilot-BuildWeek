        ...
        <div className="mt-14 grid items-center gap-14 lg:grid-cols-[1.02fr_.98fr] lg:gap-20">
          <div>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              ...
              <Link href="/demo/trust" className="button-secondary">Score Methodology</Link>
            </div>
...
function HeroArtwork() {
  return (
    <div data-testid="hero-illustration" aria-label="Illustrative JobPilot workspace showing a 100-point example score connected to candidate evidence" role="img" className="relative min-h-[570px] overflow-hidden rounded-[2.25rem] border border-white/80 bg-[#e9efe5] p-5 shadow-[0_35px_80px_rgba(37,67,52,.18)] sm:min-h-[620px] sm:p-8">
      ...
      <div className="absolute bottom-5 left-5 right-5 rounded-2xl border border-[#173d2d]/10 bg-[#fffaf0]/95 p-4 shadow-lg sm:bottom-7 sm:left-14 sm:right-8 sm:p-5">
        <div className="flex items-start justify-between gap-4">
          <div><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#a1742d]">Synthetic example role</p><p className="mt-1 font-serif text-lg leading-tight">Applied AI Solutions Engineer</p><p className="mt-2 flex items-center gap-1 text-xs text-[#667b71]"><MapPin className="size-3" /> San Francisco · Hybrid</p></div>
          <FileCheck2 className="size-5 shrink-0 text-[#4e7a63]" aria-hidden="true" />
        </div>
        <div className="mt-3 grid grid-cols-3 gap-3 border-t border-[#173d2d]/10 pt-3 text-xs"><div><span className="block text-[#73867c]">Core</span><b>88.00</b></div><div><span className="block text-[#73867c]">Preferred</span><b>9.00</b></div><div><span className="block text-[#73867c]">Nice</span><b>3.00</b></div></div>
        <p className="mt-3 text-[11px] leading-4 text-[#667b71]">Illustrative example — not the live demo result.</p>
      </div>
    </div>
  );
}
...
function EvidenceChip({ id, label }: { id: string; label: string }) {
  return <div className="relative rounded-xl border border-[#173d2d]/10 bg-[#fffdf7]/95 p-3 shadow-md"><p className="flex items-center gap-1 text-[9px] font-bold uppercase tracking-[.12em] text-[#a1742d]"><Link2 className="size-3" />{id}</p><p className="mt-1 text-[11px] font-semibold leading-4">{label}</p></div>;
}
