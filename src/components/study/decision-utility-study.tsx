...
function browserTestFromLocation() { return new URLSearchParams(window.location.search).get("validation") === "browser-test"; }
...
export function DecisionUtilityStudy() {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      ...
      setMode(nextMode); setBrowserTest(browserTestFromLocation()); setSession(saved?.phase === nextMode ? saved : null); setHydrated(true);
    }, 0);
  ...
  if (!session) return <Consent mode={mode} browserTest={browserTest} confirmed={adultConfirmed} setConfirmed={setAdultConfirmed} onConsent={() => setSession(createDecisionStudySession({ phase: mode, authenticHumanConfirmation: mode !== "PREVIEW" && !browserTest && adultConfirmed }))} />;
...
function Consent({ mode, browserTest, confirmed, setConfirmed, onConsent }: { mode: StudyMode; browserTest: boolean; confirmed: boolean; setConfirmed: (value: boolean) => void; onConsent: () => void }) {
  return <StudyShell mode={mode}><section className="mx-auto max-w-3xl py-8"><p className="eyebrow"><ShieldCheck className="size-4" /> Consent</p><h1 className="mt-4 font-serif text-5xl">Choose whether to take part.</h1><div className="paper-card mt-7 space-y-4 text-base leading-7 text-[#587064]"><p>Participation is voluntary. The study stores an anonymous ID, assigned role order, answers, completion time, confidence, and clarity in this browser until you export or reset it.</p><p>It does not ask for your name, email, phone, resume, demographic data, employment status, health information, home address, or IP address. You may exit at any time.</p><p>{browserTest ? "This browser-test session is tooling validation and will be excluded from participant evidence." : "By continuing, you consent to provide these anonymous usability responses to the study owner."}</p><label className="choice-control w-full"><input type="checkbox" checked={confirmed} onChange={(event) => setConfirmed(event.target.checked)} /> {browserTest ? "I understand this browser-test session is excluded." : "I am an adult completing this session myself."}</label><p className="text-sm">Protocol: <code>{DECISION_STUDY_PROTOCOL_VERSION}</code> · Consent: <code>{DECISION_STUDY_CONSENT_VERSION}</code></p></div><div className="mt-6 flex flex-wrap gap-3"><button type="button" disabled={!confirmed} onClick={onConsent} className="button-primary disabled:cursor-not-allowed disabled:opacity-45">{browserTest ? "Continue browser test" : "I consent and continue"}</button><Link href="/" className="button-secondary">No thanks</Link></div><LocalOnly /></section></StudyShell>;
}
...
function Completion({ mode, session, onReset }: { mode: StudyMode; session: DecisionStudySession; onReset: () => void }) {
  ...
  const labels = completionLabels(mode, session);
...
function completionLabels(mode: StudyMode, session: DecisionStudySession) {
  if (session.phase === "FINAL" && !session.consent.authenticHumanConfirmation) return { title: "Study Complete", phase: "Study", csv: "Download Study CSV", json: "Download Study JSON", supporting: "Browser-test export — excluded from participant evidence and final impact analysis." };
