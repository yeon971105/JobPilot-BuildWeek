...
function PreviewSummary({ session }: { session: DecisionStudySession }) {
  return <section className="paper-card mt-8"><h2 className="font-serif text-3xl">Preview validation summary</h2><p className="mt-2 text-sm">Assignment: <b>{session.assignmentGroup}</b> · Preview data is excluded from participant evidence.</p><div className="mt-5 grid gap-4 md:grid-cols-2">{session.responses.map((response) => { const role = DECISION_STUDY_ROLES[response.roleId]; const correctness = scoreStudyResponse(response); const warnings = response.clarity <= 3 ? "Clarity warning: review these instructions before pilot." : "No clarity warning."; return <article key={response.roleId} className="rounded-2xl bg-[#edf1e8] p-5"><h3 className="font-serif text-2xl">{role.title}</h3><p className="mt-1 text-xs font-bold">{response.condition} · {response.taskSeconds ?? 0} seconds</p><dl className="mt-4 space-y-2 text-sm">{questions.map((question) => <div key={question.key}><dt className="font-bold">{question.label}</dt><dd>{String(response[question.key]) || "No answer"} · {correctness[question.key] ? "Correct" : "Incorrect"}</dd></div>)}</dl><p className="mt-4 text-sm">Confidence {response.confidence}/7 · Clarity {response.clarity}/7</p><p className="mt-2 text-xs font-bold">{warnings}</p></article>; })}</div></section>;
}
