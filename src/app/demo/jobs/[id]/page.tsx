...
import { DEMO_CANDIDATE, DEMO_JOBS, getCachedGemmaAnalysis, getDemoJob } from "@/lib/demo-contract";
...
import type { DemoProviderMode } from "@/server/build-week/providers";
...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  const cachedGemma = getCachedGemmaAnalysis(id);
  if (!cachedGemma || cachedGemma.jobHash !== job.contentHash || cachedGemma.scoreReceiptHash !== analysis.receipt.receiptHash) throw new Error("Frozen Gemma analysis provenance does not match the deterministic receipt.");
  ...
  return <JobDetail job={job} candidate={DEMO_CANDIDATE} analysis={analysis} analysisProvider={routeHybridTask("JOB_SEMANTIC_ANALYSIS").provider as DemoProviderMode} heavyMode={getDemoProviderConfig().mode} experienceRows={experienceRows} scoreChange={buildScoreChangeScenario()} />;
}
