...
export default async function Page({ searchParams }: { searchParams: Promise<{ tour?: string }> }) {
  ...
  const analyses: JobCardAnalysis[] = DEMO_JOBS.map((job) => {
    ...
    return { jobId: job.id, score: analysis.displayedScore, evidenceQuality: analysis.evidenceQuality, evidenceSufficient: analysis.numericScoreEligibility, strongestCapability: strongest?.canonicalName ?? "Supported evidence only", largestGap: gap?.canonicalName ?? "No confirmed point loss", priority: analysis.applyPriority };
  });
