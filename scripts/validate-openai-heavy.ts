...
async function main() {
  if (!process.env.OPENAI_API_KEY?.trim()) {
    console.error(JSON.stringify({ validation: "CONFIGURATION_REQUIRED", code: 78, model: "gpt-5.6-terra", keyConfigured: false, keyPrinted: false, clearance: "Set OPENAI_API_KEY server-side and enable OPENAI_HEAVY_FEATURES_ENABLED=true." }, null, 2));
    process.exitCode = 78;
    ...
    console.log(JSON.stringify({ validation: "PASS", model: "gpt-5.6-terra", syntheticDataOnly: true, recommendation: strategy.recommendation, keyPrinted: false }, null, 2));
  }
}
