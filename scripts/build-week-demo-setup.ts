...
async function main() {
  ...
  const jobs = JSON.parse(await readFile(path.join(root, "jobs.json"), "utf8")) as Array<{ id: string; workModes: string[] }>;
  ...
  console.log(JSON.stringify({ status: "ready", synthetic: true, jobs: jobs.length, workModes: [...new Set(jobs.flatMap((job) => job.workModes))].sort(), liveGpt56Configured: Boolean(process.env.OPENAI_API_KEY && process.env.BUILD_WEEK_ALLOW_LIVE_GPT56 === "true") }, null, 2));
}
main();
